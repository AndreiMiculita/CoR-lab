...
typedef pcl::PointXYZRGBA PointT;
 ...
 /* _________________________________
  ...
  int overlap_between_contexts =5;
   
  std::string name_of_approach = "Context_Change";
...
//     {
	...
	Result  <<"Context change - system configuration:" 
		<< "\n\t-Experiment_name = " << name_of_approach 
		<< "\n\t-Dataset = " << dataset 
		<<"\n\t-Voxel size = "<<uniform_sampling_size 
		<<"\n\t-Spin image - Image width = "<<spin_image_width
		<< "\n\t-Spin image - Support length = "<< spin_image_support_lenght 
		<< "\n\t-Classification Threshold = " <<recognition_threshold << "\n\n";
...
// 	    ROS_INFO ("\t\t[-] random number = %f, contextChangeProbability = %f", random, p_context_change );
	    ...
	    ROS_INFO ("\t\t[-] random number = %i", random );
	    ...
	    if ((context_index == 0) and number_of_taught_categories > random )
	    {
	      ...
	      //class_index = int(number_of_categories_in_dataset/number_of_contexts) * (context_index); 
...
// 	    while ( ((Precision < P_Threshold ) or (k < number_of_taught_categories)) and (!User_sees_no_improvement_in_precision) )
	    ...
	    {
		int start_context_index = int(number_of_categories_in_dataset / number_of_contexts) * (context_index);
		...
		if (!inContext(c.at(Ci), context_index, number_of_contexts, overlap_between_contexts, number_of_categories_in_dataset))
		{
		    Ci++;
		    continue;
		}

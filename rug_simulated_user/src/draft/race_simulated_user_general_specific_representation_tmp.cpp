...
#include <LDA_based_object_recognition/LDA_based_object_recognition_functionality.h>
...
typedef pcl::PointXYZRGBA PointT;
 ...
 /* _________________________________
  ...
  |_________________________________| */
    ...
    std::string name_of_approach = "TPAMI_LOCAL_LDA";
...
// 	/home/hamidreza/Objectrecognition_code/raceua/race_object_representation/ICRA_dictionaries/clusters7048.txt
	...
	int total_number_of_train_object=0;
	...
	/// Create a new dictionary using original dictionary and phi matrix
	vector <SITOV> generic_dictionary;
	createNewDictionaryFromPhi(dictionary, phi, generic_dictionary );
	ROS_INFO ("generic_dictionary.size() = %i", generic_dictionary.size());
	VisualizeNewDictionaryInMatlab(generic_dictionary, spin_image_width, spin_image_support_lenght);
...
// 	    while ( ((Precision < P_Threshold ) or (k < number_of_taught_categories)) and (!User_sees_no_improvement_in_precision) )
		...
		else
		{
		    ...
		    int total_number_of_train_objects = number_of_instances;      
		    ROS_INFO("\t\t[-] total_number_of_train_objects : %d", total_number_of_train_objects);

		    //info for debug
// 		    ROS_INFO("\t\t[-] total_number_of_gibbs_sampling_iterations : %i", total_number_of_gibbs_sampling_iterations);
// 		    ROS_INFO("\t\t[-] total_number_of_topic : %i", total_number_of_topic);
// 		    ROS_INFO("\t\t[-] alpha : %lf", alpha);
// 		    ROS_INFO("\t\t[-] beta : %lf", beta);
// 		    ROS_INFO("\t\t[-] uniform_sampling_size : %lf", uniform_sampling_size);
// 		    ROS_INFO("\t\t[-] dictionary size : %d", dictionary.size());
		  ...
		  /*_______________________________
		    |                                |
		    |     initialisation of LDA      |
		    |________________________________| */
		    initialisation( dictionary, 
				    total_number_of_topic,
				    total_number_of_train_objects,
				    total_number_of_words_in_each_object,
				    total_number_of_words_for_topic,
				    sample_topic_index,
				    object_topic_matrix,
		    ...
		    ROS_INFO ("LDA initialized... ");

		    std::string test_instance;

		    //extract ground truth category name
		    Object_name_orginal = test_instance;	   
		    ...
		    //NOTE: the test category name shoud start with smal z ;
		    conceptualizeObjectViewSpinImagesInSpecificCategory("zzzzzzzTEST",1,1000,1,*objectViewSpinImages,pp);
		    
		    vector <int> new_total_number_of_words_for_topic2;
		    for (int i =0; i < total_number_of_words_for_topic.size(); i++)
		    {
		      new_total_number_of_words_for_topic2.push_back(total_number_of_words_for_topic.at(i) + new_total_number_of_words_for_topic.at(i));
		    }
		    ...
		    sample_topic_index.push_back(new_sample_topic_index.at(0));
		    object_topic_matrix.push_back(new_object_topic_matrix.at(0));
		    theta.push_back(new_theta.at(0));
		    phi.push_back(new_phi.at(0));
		    ...
		    estimateLDA( total_number_of_gibbs_sampling_iterations,
				  alpha /*constant*/,
				  beta /*constant*/,
				  dictionary, 
		  ...
		  /* __________________________
		    ...
		    vector <double> tmp;
		    for (int i = 0; i < theta.at(0).size(); i++)
		    {
		      tmp.push_back(0);
		    } 
		    for (int i = 0; i < theta.at(0).size(); i++)
		    {
		      double tmp1 = theta.at(theta.size()-1).at(i);
		      tmp.at(i)= tmp1;
		    } 
		    
		    int match_index= 0;
		    float maximum_likelihood =1000;
		    for (int k = 0; k < theta.size()-1; k ++)
		    {
			double similarity;
			
			//KL distance
			 kullbackLiebler2( theta.at(k), tmp, similarity );  
			//Euclidean distance
			//euclidean_distance ( theta.at(k), tmp, similarity); 
			
			printf (" %2.4f, " , similarity); 
			if (similarity < maximum_likelihood)
			{
			    match_index = k; // track_id is strated from 1 ;
			    maximum_likelihood = similarity;  
			} 
		    }

		    //create an object_view_key 
		    //std::string predicted_view_key = _pdb->makeKey(key::RV, match_index+1 /*track_id is sarted from 1*/, 1);
		    ...
		    string result ;
		    int count =0;
		    for (int l = 0; l < ListOfObjectCategory.size(); l++ )
		    { 
		      for (int m = 0; m < ListOfObjectCategory.at(l).rtov_keys.size(); m++ )
		      { 
			string object_view_key = ListOfObjectCategory.at(l).rtov_keys.at(m).c_str();
			//ROS_INFO("\t\t[-]- train OVK: %s", object_view_key.c_str());
			if (match_index == count)
			{
			    //ROS_INFO("\t\t[-]- train OVK: %s",ListOfObjectCategory.at(l).rtov_keys.at(m).c_str());
			    result = ListOfObjectCategory.at(l).cat_name.c_str();
			    //break;
			}
			count++;
		      } 
		    } 
		    ROS_INFO("count =  %i , matched_index = %i and ML = %f , result = %s", count, match_index, maximum_likelihood, result.c_str());

	  // 	    ROS_INFO("result_predictive_likelihood_ranking = %s", result.c_str());
		    ...
		    _rrtov.minimum_distance =maximum_likelihood;
			
		    pub.publish (_rrtov);
// 		    _p_recognitionResult_publisher->publish (_rrtov);

		    duration = ros::Time::now() - beginProc;
		    duration_sec = duration.toSec();
		    ROS_INFO( "recognition tooks %f secs", duration_sec);

		      
		    //remove the test data form database and train data
		    deleteObjectViewSpinImagesFromSpecificCategory("zzzzzzzTEST",1,1000,1,objectViewSpinImages);
		    
		    ListOfObjectCategory = _pdb->getAllObjectCat();
		    //ROS_INFO("%d categories exist in the perception database",  ListOfObjectCategory.size());
		    ...
		    total_number_of_words_in_each_object.pop_back();
		    sample_topic_index.pop_back();
		    object_topic_matrix.pop_back();
		    theta.pop_back();
		    phi.pop_back();

...
//   Purpose: 	(naive bayes leraning approach)
//   		This program usges naive bayes classifier and follows the teaching 
//   		protocol and autonomously interact with the system using teach, ask
//   		and correct actions. 
...
/* _________________________________
   ...
   |_________________________________| */
    ...
    //rm -rf /tmp/pdb
    //roslaunch race_simulated_user simulated_user_using_naive_bayes.launch
...
/* _________________________________
 ...
 |_________________________________| */
    ...
    double uniform_sampling_size = 0.01;
    ...
    std::string name_of_approach = "GOOD_Grasp";
...
/* _________________________________
  ...
  |_________________________________| */
    ...
    std::ofstream Result , PrecisionMonitor, category_introduced,  local_F1_vs_category, F1_vs_category;
...
void evaluationfunction(const race_perception_msgs::RRTOV &result)
{
	...
	number_of_instances ++;
// 	
// 	IntroduceNewInstanceHistogramNotNormalized ( InstancePathTmp, 
// 						     cat_id, 
// 						     track_id, 
// 						     view_id,
// 						     pp,
...
// 						   );
	...
	IntroduceNewInstanceHistogramNotNormalized2 ( InstancePathTmp, 
							cat_id, 
							track_id, 
							view_id,
							pp,
	...
	updateNaiveBayesModel(True_cat,cat_id, pp);
...
// 	updateNaiveBayesModel(True_Category_Global,cat_id, pp);
	updateNaiveBayesModel(True_cat,cat_id, pp);
...
{
    ROS_INFO("Simulated user using naive bayes -> hello world");
    int RunCount=1;
    PrettyPrint pp;
...
//     {
	...
	system( systemStringCommand.c_str());
        ...
        ///storing Naive bayes model
	systemStringCommand= "mkdir "+ ros::package::getPath("race_naive_bayes_object_recognition")+ "/NaiveBayesModels" ;
	system( systemStringCommand.c_str());
...
// 	ros::Publisher pub = nh.advertise< race_perception_msgs::PCTOV> (pcin_topic, 1000);
	...
	ros::Publisher _p_crtov_publisher = nh.advertise<race_perception_msgs::CompleteRTOV> ("/perception/pipeline_default/object_representation/histogram_tracked_object_view", 1000);		
...
// 	while (ros::ok() && (ros::Time::now() - start_time).toSec() <5)
// 	{  //wait  
// 	}
...
// 	introduceNewCategoryHistogramAndUpdatingNaiveBayesModel(class_index,
// 								  track_id,
...
// 							      );
// 		
	introduceNewCategoryHistogramAndUpdatingNaiveBayesModel2( class_index,
								    track_id,
								    instance_number2.at(class_index-1),
								    evaluationFile,
								    pp,
	...
	while ( class_index < number_of_categories)  // one category already taught above
	{
	    ...
	    InstancePath= "";
// 	    if (introduceNewCategoryHistogramAndUpdatingNaiveBayesModel(class_index,
// 									track_id,
// 									instance_number2.at(class_index-1),
// 									evaluationFile,
// 									pp,
...
// 									) ==-1)
	    ...
	    if (introduceNewCategoryHistogramAndUpdatingNaiveBayesModel2(class_index,
									track_id,
									instance_number2.at(class_index-1),
									evaluationFile,
									pp,
	...
	/* _____________________________________________________
	  ...
	  |_____________________________________________________| */
	   ...
	   //Declare a boost share ptr to the spin image msg
	    ...
	    |                                                         |
	    |  send test object representation to object recognition  |
...
// 		if (InstancePath.size() < 2) 
// 		{
// 		    ROS_INFO("\t\t[-]- The experiment terminates because The %s file does not exist.", InstancePath.c_str());
// 		    category_introduced.close();
// 		    ROS_INFO("\t\t[-]- Number of taught categories = %i", number_of_taught_categories); 
// 		    ros::Duration duration = ros::Time::now() - beginProc;
// 		    report_current_results(TP,FP,FN,evaluationFile,true);
...
// 		    pub.publish (msg);
// 		    ROS_INFO("\t\t[-]- Emulating race_object_tracking pakage by publish a point cloud: %s", InstancePath.c_str());		        		    // wait
// 		    
// 		    start_time = ros::Time::now();
// 		    while (ros::ok() && (ros::Time::now() - start_time).toSec() < 1)
// 		    { //wait
